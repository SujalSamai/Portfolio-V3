...
function MainProjectCard(props) {
  const img = (
    <div className="w-[65%] hover:transition ease-out duration-500 cursor-pointer shadow-lg hover:-translate-y-4">
      <a href={props.live}>
        <img
  ...
  if (props.index === 2) {
    return (
      <div className="flex py-16">
        <div className="w-2/4 z-10 text-slate-200 p-4">
    ...
    return (
      <div className="flex py-16">
        {img}
        <div className="w-2/4 z-10 text-slate-200 p-4 text-right">
          <p className="text-gr font-martian text-xs">Featured Project</p>
          <h2 className="text-slate-300 text-3xl font-bold mb-4">
            {props.heading}
          </h2>
...
function MainProjects() {
  ...
  return (
    <section id="work" className="w-7/12 mx-auto my-[15rem] flex flex-col ">
      <div className="w-10/12 flex justify-between items-center">
        <h1 className="text-2xl text-slate-300 font-inter font-semibold">
          <span className="font-martian text-gr text-lg">03. </span>Some things
          I've Built
        </h1>
        <p className="w-6/12 border-t-[1px] border-slate-600"></p>
      </div>
      <div>{projectsEl}</div>
    </section>
  );
